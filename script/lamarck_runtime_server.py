...
class DMA_Transmitter(object):
    ...
    def send_flit_bin(self, flit_bin):
        ...
        if length > DMA_BUF_MAX:                                           
            print("===<2>=== %s is larger than %dMB" % (flit_bin, DMA_BUF_MAX>>20))
...
def start_tcp_server(ip, port, id):
    ...
    try:
        ...
        print("fail to listen on ip %s, port %s" % server_address)
        sys.exit(1)
    receive_count = 0
    ...
    while True:
        ...
        while True:
            ...
            if length == 0:
                ...
                recv_len -= 8
                start = 8
                if data_type == SET_FREQUENCY:
                    print("freq=%dM" % length)
                else:
                ...
                if data_type == CHIP_RESET:
                    ...
                    length = 0
                    trans.close()
                    # os.system(darwin3_runtime_node_dir + "/script/chip_reset")
                    os.system("bash " + darwin3_runtime_node_dir + "/script/restart_dma")
                    os.system("bash " + darwin3_runtime_node_dir + "/script/restart_dma")
                    trans.open()
                elif data_type == SET_FREQUENCY:
                    freq = length
                    length = 0
                    trans.close()
                    ...
                    # subprocess.run(["/home/root/scripts/flits_sender", f"/home/root/chip_test/reset_clock/pll_clock_{freq}M.bin"],check=True)  # 如果命令失败则引发异常
                    trans.open()
                #f.write(msg[8:])
                if length > 0:
...
if __name__=='__main__':
    ...
    while(True):
        ...
        if len(sys.argv)>1:
            id = int(sys.argv[1])
        try:
            server_thread1 = threading.Thread(target=start_tcp_server, args=('0.0.0.0',6000+id,id))
        ...
        id = 1 
        try:
            server_thread2 = threading.Thread(target=start_tcp_server, args=('0.0.0.0',6000+id,id))
